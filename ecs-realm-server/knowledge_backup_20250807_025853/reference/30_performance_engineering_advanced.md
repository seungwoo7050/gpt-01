...
public:
    ...
    float DotProduct(const std::vector<float>& a, const std::vector<float>& b) {
        ...
        __m256 vsum = _mm256_setzero_ps();  // 0으로 초기화
        ...
        _mm256_store_ps(sum_array, vsum);
...
public:
    ...
    void UpdatePositionsSIMD(PlayerPositions& positions, float delta_time) {
        // 델타 타임을 8개로 복제
        __m256 vdelta = _mm256_set1_ps(delta_time);
        
        // X 좌표 업데이트: x = x + vx * dt (8개 동시)
        __m256 vx_pos = _mm256_load_ps(positions.x);
        __m256 vx_vel = _mm256_load_ps(positions.vx);
        __m256 vx_new = _mm256_fmadd_ps(vx_vel, vdelta, vx_pos);  // FMA: a*b+c
        _mm256_store_ps(positions.x, vx_new);
        
        // Y 좌표 업데이트
        __m256 vy_pos = _mm256_load_ps(positions.y);
        __m256 vy_vel = _mm256_load_ps(positions.vy);
        __m256 vy_new = _mm256_fmadd_ps(vy_vel, vdelta, vy_pos);
        _mm256_store_ps(positions.y, vy_new);
        
        // Z 좌표 업데이트
        __m256 vz_pos = _mm256_load_ps(positions.z);
        __m256 vz_vel = _mm256_load_ps(positions.vz);
        __m256 vz_new = _mm256_fmadd_ps(vz_vel, vdelta, vz_pos);
        _mm256_store_ps(positions.z, vz_new);
    }
    ...
    void CalculateDistancesSIMD(const PlayerPositions& players, 
                               ...
                               float distances[8]) {
        __m256 vtarget_x = _mm256_set1_ps(target_x);
        __m256 vtarget_y = _mm256_set1_ps(target_y);
        __m256 vtarget_z = _mm256_set1_ps(target_z);
        
        __m256 vplayers_x = _mm256_load_ps(players.x);
        __m256 vplayers_y = _mm256_load_ps(players.y);
        __m256 vplayers_z = _mm256_load_ps(players.z);
        
        // 차이 계산
        __m256 vdiff_x = _mm256_sub_ps(vplayers_x, vtarget_x);
        __m256 vdiff_y = _mm256_sub_ps(vplayers_y, vtarget_y);
        __m256 vdiff_z = _mm256_sub_ps(vplayers_z, vtarget_z);
        
        // 제곱
        __m256 vsq_x = _mm256_mul_ps(vdiff_x, vdiff_x);
        __m256 vsq_y = _mm256_mul_ps(vdiff_y, vdiff_y);
        __m256 vsq_z = _mm256_mul_ps(vdiff_z, vdiff_z);
        
        // 제곱합
        __m256 vsum = _mm256_add_ps(_mm256_add_ps(vsq_x, vsq_y), vsq_z);
        
        // 제곱근 (8개 동시)
        __m256 vdistances = _mm256_sqrt_ps(vsum);
        
        _mm256_store_ps(distances, vdistances);
    }
    ...
    void CheckLineOfSightSIMD(const PlayerPositions& players,
        ...
        __m256 vdistances = _mm256_load_ps(distances);
        __m256 vmax_dist = _mm256_set1_ps(max_distance);
        
        // 거리 비교 (8개 동시)
        __m256 vmask = _mm256_cmp_ps(vdistances, vmax_dist, _CMP_LE_OQ);
...
public:
    ...
    __m256 FastInvSqrt(__m256 x) {
        __m256 x_half = _mm256_mul_ps(x, _mm256_set1_ps(0.5f));
        ...
        // 뉴턴-랩슨 반복 (2회)
        y = _mm256_mul_ps(y, _mm256_fnmsub_ps(x_half, _mm256_mul_ps(y, y), _mm256_set1_ps(1.5f)));
        y = _mm256_mul_ps(y, _mm256_fnmsub_ps(x_half, _mm256_mul_ps(y, y), _mm256_set1_ps(1.5f)));
    ...
    void FastSinCos(__m256 x, __m256& sin_result, __m256& cos_result) {
        // x를 [-π, π] 범위로 정규화
        __m256 pi = _mm256_set1_ps(3.14159265359f);
        ...
        sin_result = _mm256_sub_ps(
            _mm256_add_ps(x, _mm256_mul_ps(x7, _mm256_set1_ps(-1.0f/5040.0f))),
            _mm256_mul_ps(x3, _mm256_set1_ps(1.0f/6.0f))
        );
        sin_result = _mm256_add_ps(sin_result, _mm256_mul_ps(x5, _mm256_set1_ps(1.0f/120.0f)));
        
        // 코사인 계산 (sin의 위상 이동 버전)
        __m256 half_pi = _mm256_set1_ps(1.57079632679f);
        __m256 cos_x = _mm256_sub_ps(half_pi, x);
        
        __m256 cos_x2 = _mm256_mul_ps(cos_x, cos_x);
        __m256 cos_x3 = _mm256_mul_ps(cos_x2, cos_x);
        __m256 cos_x5 = _mm256_mul_ps(cos_x3, cos_x2);
        __m256 cos_x7 = _mm256_mul_ps(cos_x5, cos_x2);
        
        cos_result = _mm256_sub_ps(
            _mm256_add_ps(cos_x, _mm256_mul_ps(cos_x7, _mm256_set1_ps(-1.0f/5040.0f))),
            _mm256_mul_ps(cos_x3, _mm256_set1_ps(1.0f/6.0f))
        );
        cos_result = _mm256_add_ps(cos_result, _mm256_mul_ps(cos_x5, _mm256_set1_ps(1.0f/120.0f)));
    }
};
```
...
void ProcessBadData(BadAlignment* data) {
    ...
    __m256 result = _mm256_load_ps((float*)&data->simd_data);  // 크래시 가능!
}
...
void ProcessGoodData(GoodAlignment* data) {
    // 정렬된 메모리 접근 - 최고 성능
    __m256 result = _mm256_load_ps((float*)&data->simd_data);
}
```
...
void BadOptimization(float* d, const float* a, const float* b, int n) {
    for(int i=0;i<n;i+=8)_mm256_store_ps(d+i,_mm256_add_ps(_mm256_load_ps(a+i),_mm256_load_ps(b+i)));
}
...
void GoodOptimization(float* dest, const float* src1, const float* src2, int count) {
    ...
    for (int i = 0; i < simd_count; i += SIMD_WIDTH) {
        __m256 vec1 = _mm256_load_ps(&src1[i]);
        __m256 vec2 = _mm256_load_ps(&src2[i]);
