...
public:
    ...
    static GameEvent CreateLevelUpEvent(uint32_t player_id, int old_level, 
                                      int new_level, uint64_t experience) {
        GameEvent event;
...
class AnalyticsService {
private:
    std::atomic<uint64_t> total_kills_{0};
    std::atomic<uint64_t> total_levelups_{0};
...
public:
    // 이벤트 저장
    uint64_t AppendEvent(const GameEvent& event) {
    ...
    // 특정 사용자의 이벤트 스트림 조회
    std::vector<GameEvent> GetEventStream(uint32_t user_id, uint64_t from_version = 0) {
        std::lock_guard<std::mutex> lock(events_mutex_);
        ...
        for (const auto& event : events_) {
            if (event.user_id == user_id) {
                uint64_t event_version = event.payload.value("_version", 0);
    ...
    // 전체 이벤트 스트림 조회 (시간 범위)
    std::vector<GameEvent> GetEventsByTimeRange(uint64_t start_time, uint64_t end_time) {
        std::lock_guard<std::mutex> lock(events_mutex_);
...
public:
    struct PlayerSnapshot {
        ...
        uint64_t snapshot_version;  // 이 스냅샷이 만들어진 시점의 버전
        uint64_t timestamp;
...
private:
    ...
    // 스냅샷 생성 간격 (이벤트 수 기준)
    const uint64_t SNAPSHOT_INTERVAL = 100;
...
public:
    ...
    // 스냅샷이 필요한지 확인
    bool ShouldCreateSnapshot(uint32_t player_id, uint64_t current_version) {
...
public:
    ...
    void ProcessEvent(const GameEvent& event) {
        // 1. 이벤트 저장
        uint64_t version = event_store_.AppendEvent(event);
    ...
    PlayerProjection::PlayerState GetPlayerStateAtTime(uint32_t player_id, uint64_t target_time) {
...
private:
    ...
    uint64_t max_wait_time_ms_ = 5000;  // 5초 최대 대기
...
public:
    ...
    void ProcessMessage(uint64_t sequence, const GameEvent& event) {
        {
            std::lock_guard<std::mutex> lock(queue_mutex_);
        ...
        while (running_) {
            ...
            while (!message_queue_.empty()) {
                ...
                if (top_msg.sequence_number == expected_sequence_) {
                    ...
                    // 미래 메시지 - 대기
                    uint64_t wait_time = now - top_msg.timestamp;
...
public:
    ...
    void ProcessMessage(uint32_t partition_key, uint64_t sequence, const GameEvent& event) {
