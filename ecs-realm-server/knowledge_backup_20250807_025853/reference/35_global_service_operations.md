...
public:
    ...
    void MigrateUserToRegion(uint64_t user_id, 
    ...
    // 친구 시스템 - 크로스 리전 지원
    void AddFriend(uint64_t user_id, uint64_t friend_id) {
...
private:
    struct UserData {
        ...
        std::vector<uint64_t> friend_list;
        uint64_t guild_id;
    };
    
    UserData CollectUserData(uint64_t user_id, const std::string& region) {
        UserData data;
    ...
    void AddFriendCrossRegion(uint64_t user_id, uint64_t friend_id,
...
public:
    // 사용자 동의 관리
    void ProcessUserConsent(uint64_t user_id, const std::map<std::string, bool>& consents) {
    ...
    // 개인정보 삭제 요청 (잊힐 권리)
    void ProcessDeletionRequest(uint64_t user_id, const std::string& request_reason) {
    ...
    // 데이터 포팅 요청 (데이터 이동권)
    nlohmann::json ProcessDataPortabilityRequest(uint64_t user_id) {
...
private:
    ...
    DeletionAssessment AssessDeletionFeasibility(uint64_t user_id) {
    ...
    std::vector<DataLocation> DiscoverUserData(uint64_t user_id) {
    ...
    void ExecuteDataDeletion(uint64_t user_id, const std::vector<DataLocation>& locations) {
