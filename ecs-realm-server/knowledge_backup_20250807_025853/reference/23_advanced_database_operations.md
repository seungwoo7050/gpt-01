...
public:
    ...
    virtual int GetShardIndex(uint64_t key, int shard_count) const = 0;
    virtual std::string GetStrategyName() const = 0;
    virtual void AnalyzeDistribution(const std::vector<uint64_t>& keys, int shard_count) const = 0;
};

// 1️⃣ 해시 기반 샤딩 (가장 일반적)
class HashSharding : public ShardingStrategy {
public:
    int GetShardIndex(uint64_t key, int shard_count) const override {
        return std::hash<uint64_t>{}(key) % shard_count;
    }
    ...
    void AnalyzeDistribution(const std::vector<uint64_t>& keys, int shard_count) const override {
...
private:
    std::vector<uint64_t> range_boundaries;
    
public:
    RangeSharding(const std::vector<uint64_t>& boundaries) : range_boundaries(boundaries) {}
    ...
    void AnalyzeDistribution(const std::vector<uint64_t>& keys, int shard_count) const override {
        ...
        for (int i = 0; i < shard_count; ++i) {
            float percentage = (float)distribution[i] / keys.size() * 100;
            uint64_t range_start = (i == 0) ? 0 : range_boundaries[i-1] + 1;
            uint64_t range_end = range_boundaries[i];
    ...
    ConsistentHashing(int shard_count, int virtual_nodes = 150) 
        ...
        for (int shard = 0; shard < shard_count; ++shard) {
            for (int vnode = 0; vnode < virtual_nodes_per_shard; ++vnode) {
                ...
                uint64_t hash_val = std::hash<std::string>{}(vnode_key);
...
public:
    void AnalyzeShardingStrategies() {
        ...
        // 3) 클러스터된 가입 (이벤트 기간)
        for (uint64_t base = 1000000; base <= 1010000; base += 100) {
            for (int i = 0; i < 20; ++i) {
        ...
        std::vector<uint64_t> range_boundaries = {125000, 250000, 375000, 500000, 625000, 750000, 875000, UINT64_MAX};
...
public:
    ...
    // 🎮 플레이어 데이터 샤드별 저장
    bool SavePlayerData(uint64_t player_id, const std::string& player_name, 
                       int level, int64_t experience, const std::string& location_data) {
    ...
    // 🎮 플레이어 데이터 샤드에서 로드
    std::optional<PlayerData> LoadPlayerData(uint64_t player_id) {
        int shard_index = sharding_strategy_->GetShardIndex(player_id, shards_.size());
        auto& shard = shards_[shard_index];
...
('20240115_add_guild_id', 'Add guild_id column to players table', 
 'ALTER TABLE players DROP COLUMN guild_id;'),
('20240116_partition_game_logs', 'Partition game_logs table by year',
...
private:
    ...
    uint64_t last_binlog_position_ = 0;
...
private:
    ...
    void ParseBinaryLogEvents() {
        ...
        uint64_t current_position = result[0]["Position"];
...
class BadShardingQuery {
    void GetPlayerWithInventory(uint64_t player_id) {
