...
int main() {
    ...
    // 플레이어 ID (정수)
    uint64_t player_id = 12345;           // 64비트 양의 정수 (아주 큰 숫자)
...
**💡 게임 서버에서 중요한 이유:**
- `uint64_t`: 수십억 명의 플레이어 ID도 저장 가능
...
int main() {
    // 🎮 플레이어 ID와 이름을 연결하는 맵
    std::map<uint64_t, std::string> player_names;
