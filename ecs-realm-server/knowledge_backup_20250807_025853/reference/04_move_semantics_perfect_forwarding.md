...
public:
    // 1. 생성자
    ModernPlayer(uint64_t id, const std::string& name, int level)
        : id_(id)
...
public:
    // 생성자
    PimplPlayer(uint64_t id, const std::string& name, int level);
...
public:
    ...
    PlayerImplementation(uint64_t id, const std::string& name, int level)
...
PimplPlayer::PimplPlayer(uint64_t id, const std::string& name, int level)
...
// 접근자 구현
uint64_t PimplPlayer::GetId() const {
...
1. **[빌드 시스템 고급](./34_build_systems_advanced.md)** 🔥
   - CMake 고급 패턴과 최적화
   - 컴파일 시간 단축 기법

2. **[테스팅 프레임워크 완전 가이드](./35_testing_frameworks_complete.md)** 🔥
   - Google Test/Mock 마스터
   - 성능 테스트와 벤치마킹

3. **[디버깅과 프로파일링 도구](./36_debugging_profiling_toolchain.md)** 🔥
