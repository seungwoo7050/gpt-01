...
public:
    PlayerNotFoundException(uint64_t player_id)
        : PlayerException("Player with ID " + std::to_string(player_id) + " not found")
        , player_id_(player_id) {}
    
    uint64_t GetPlayerId() const noexcept { return player_id_; }
};
...
private:
    void LoginPlayer(uint64_t id, const std::string& username, const std::string& password) {
...
public:
    ...
    public:
        ...
        Player* GetPlayer(uint64_t player_id) noexcept {
            std::lock_guard<std::mutex> lock(players_mutex_);
    ...
    private:
        void NotifyPlayerAdded(uint64_t player_id) {
        ...
        void LogError(const std::string& format, uint64_t id, const std::string& msg) {
...
public:
    ...
    // 안전한 게임 데이터 저장
    Task<bool> SaveGameDataAsync(uint64_t player_id) {
        try {
            // 1단계: 현재 플레이어 상태 수집
    ...
    Task<std::vector<bool>> ProcessMultiplePlayersAsync(const std::vector<uint64_t>& player_ids) {
...
private:
    ...
    Task<void> UpdateLastLoginAsync(uint64_t player_id) {
        // 마지막 로그인 시간 업데이트
        co_return;
    }
    
    Task<std::optional<PlayerData>> CollectPlayerDataAsync(uint64_t player_id) {
        // 플레이어 현재 상태 수집
    ...
    Task<void> EmergencyLocalSaveAsync(uint64_t player_id) {
        // 응급 로컬 저장
...
public:
    ...
    int ProcessPlayerWithExceptions(uint64_t player_id) {
        if (player_id == 0) {
    ...
    // ✅ 에러 코드 기반 (안정적인 성능)
    std::pair<ErrorCode, int> ProcessPlayerWithErrorCodes(uint64_t player_id) noexcept {
        if (player_id == 0) {
    ...
    std::optional<int> ProcessPlayerWithOptional(uint64_t player_id) noexcept {
    ...
    /*
    std::expected<int, ErrorCode> ProcessPlayerWithExpected(uint64_t player_id) noexcept {
        if (player_id == 0) {
    ...
    */
    ...
    void PerformanceComparison() {
        ...
        const std::vector<uint64_t> test_ids = {1, 500, 1000, 2000, 500000, 1000000};
    ...
    public:
        ...
        bool ProcessPlayerFast(uint64_t player_id, int& result) noexcept {
            // 빠른 경로: 기본적인 검증만
            if (player_id == 0) {
        ...
        int ProcessPlayerWithValidation(uint64_t player_id) {
    ...
    private:
        void ValidatePlayerIdStrict(uint64_t player_id) {
            if (player_id == 0) {
        ...
        void ValidatePlayerExistence(uint64_t player_id) {
            if (player_id % 1000 == 0) {
                throw PlayerNotFoundException(player_id);
            }
        }
        
        void ValidatePlayerPermissions(uint64_t player_id) {
            if (player_id % 100 == 0) {
    ...
    public:
        ...
        bool SafeProcessPlayer(uint64_t player_id, int& result) noexcept {
            // 1단계: 빠른 사전 검사
        ...
        void ThrowDetailedException(uint64_t player_id, const std::string& operation) {
    ...
    private:
        bool IsPlayerIdValid(uint64_t player_id) const noexcept {
            return player_id > 0 && player_id <= 1000000;
        }
        
        int DoProcessPlayer(uint64_t player_id) {
            if (player_id % 1000 == 0) {
...
private:
    ...
    void NotifyPlayerError(uint64_t player_id, const std::string& error) noexcept {
...
2. **[Move 의미론과 Perfect Forwarding](./33_move_semantics_perfect_forwarding.md)** 🔥
   - 예외 안전한 이동 의미론
   - 완벽한 전달과 성능 최적화

3. **[빌드 시스템 고급](./34_build_systems_advanced.md)** 🔥
   - CMake 고급 패턴
   - 테스트 프레임워크 통합
