...
public:
    ...
    // 게임 서버 특화 집합 연산 예제들
    std::vector<uint64_t> GetAllUniquePlayerIds(const std::vector<uint64_t>& online_players,
                                               const std::vector<uint64_t>& offline_players) {
    ...
    std::vector<uint64_t> FindPlayersInBothEvents(const std::vector<uint64_t>& event1_participants,
                                                 const std::vector<uint64_t>& event2_participants) {
        std::vector<uint64_t> common_participants;
    ...
    bool IsGuildSubsetOfAnother(const std::vector<uint64_t>& small_guild,
                               const std::vector<uint64_t>& large_guild) {
        // 작은 길드의 모든 멤버가 큰 길드에 포함되어 있는지 확인
        return std::includes(large_guild.begin(), large_guild.end(),
                           small_guild.begin(), small_guild.end());
    }
    
    void AnalyzePvPParticipation(const std::vector<uint64_t>& all_players,
                                const std::vector<uint64_t>& pvp_players) {
        // PvP에 참여하지 않은 플레이어들
        std::vector<uint64_t> non_pvp_players;
...
**다음 문서**: `33_stl_algorithms_advanced.md`에서 고급 STL 알고리즘과 최적화 기법을 계속 학습합니다.