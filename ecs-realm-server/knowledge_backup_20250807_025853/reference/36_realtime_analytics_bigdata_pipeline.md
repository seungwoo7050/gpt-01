...
struct GameEvent {
    ...
    uint64_t user_id;            // 사용자 ID
    std::string session_id;       // 세션 ID
    uint64_t timestamp_ms;        // 타임스탬프 (밀리초)
...
public:
    static GameEvent CreateLoginEvent(uint64_t user_id, const std::string& session_id) {
        GameEvent event;
    ...
    static GameEvent CreatePurchaseEvent(uint64_t user_id, const std::string& session_id,
    ...
    static GameEvent CreateBattleEvent(uint64_t attacker_id, uint64_t defender_id,
...
private:
    ...
    struct RealTimeMetrics {
        std::atomic<uint64_t> concurrent_users{0};
        std::atomic<uint64_t> events_per_second{0};
        std::atomic<double> average_session_length{0};
        std::atomic<uint64_t> total_revenue_today{0};
        
        std::unordered_map<std::string, std::atomic<uint64_t>> events_by_type;
        std::unordered_map<std::string, std::atomic<uint64_t>> users_by_platform;
...
private:
    ...
    void DetectSuspiciousActivity(uint64_t user_id, const std::string& skill_id, uint64_t timestamp) {
        // 매크로 사용 의심 행동 감지
        static std::unordered_map<uint64_t, std::vector<uint64_t>> user_skill_timings;
