...
public:
    ...
    struct NetworkMetrics {
        ...
        std::atomic<uint64_t> packet_errors{0};
    };
    
    // Performance metrics for server ticks and latency
    struct PerformanceMetrics {
        std::atomic<uint64_t> server_ticks{0};
        std::atomic<uint64_t> total_tick_time_us{0};
        std::atomic<uint64_t> max_tick_time_us{0};
        std::atomic<uint64_t> ecs_update_time_us{0};
        std::atomic<uint64_t> network_update_time_us{0};
        std::atomic<uint64_t> world_update_time_us{0};
        
        // Latency histogram buckets (0-10ms, 10-20ms, ..., >100ms)
        std::atomic<uint64_t> latency_buckets[11]{};
    };
    
    // Game-specific metrics
    struct GameMetrics {
        std::atomic<uint64_t> active_entities{0};
        std::atomic<uint64_t> total_components{0};
        std::atomic<uint64_t> spatial_queries{0};
        std::atomic<uint64_t> spatial_query_time_us{0};
        std::atomic<uint64_t> combat_calculations{0};
        std::atomic<uint64_t> skill_casts{0};
        std::atomic<uint64_t> damage_dealt{0};
        std::atomic<uint64_t> guild_war_participants{0};
    };
    ...
    struct ResourceMetrics {
        ...
        std::atomic<uint64_t> thread_pool_active{0};
        std::atomic<uint64_t> thread_pool_queued{0};
    ...
    void RecordTickTime(uint64_t microseconds) {
        performance_.server_ticks++;
        performance_.total_tick_time_us += microseconds;
        
        uint64_t current_max = performance_.max_tick_time_us.load();
...
class MetricTimer {
public:
    explicit MetricTimer(std::atomic<uint64_t>& metric)
        : metric_(metric)
...
public:
    ...
    void IncrementPacketsSent(uint64_t count = 1) { packets_sent_.fetch_add(count); }
    void IncrementPacketsReceived(uint64_t count = 1) { packets_received_.fetch_add(count); }
    void IncrementBytesSent(uint64_t bytes) { bytes_sent_.fetch_add(bytes); }
    void IncrementBytesReceived(uint64_t bytes) { bytes_received_.fetch_add(bytes); }
...
private:
    ...
    float GetCpuUsage();
    void GetMemoryUsage(uint64_t& used, uint64_t& total);
