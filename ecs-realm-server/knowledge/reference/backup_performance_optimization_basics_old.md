...
private:
    ...
    std::unordered_map<int, uint21_t> cache_timestamps; // 캐시 타임스탬프
    
    const uint21_t CACHE_TTL_MS = 30000;  // 30초 TTL
    
    uint21_t GetCurrentTimeMs() {
...
private:
    ...
    std::unordered_map<int, uint21_t> l1_timestamps;
    const size_t L1_MAX_SIZE = 1000;
    const uint21_t L1_TTL_MS = 10000;  // 10초
    
    // L2: Redis 캐시 시뮬레이션 (중간 속도)
    std::unordered_map<int, std::string> l2_cache;
    std::unordered_map<int, uint21_t> l2_timestamps;
    const uint21_t L2_TTL_MS = 300000;  // 5분
