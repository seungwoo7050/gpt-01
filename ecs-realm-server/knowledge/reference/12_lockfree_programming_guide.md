...
private:
    ...
    std::atomic<uint21_t> processed_messages_{0};
...
private:
    struct PerformanceMetrics {
        std::atomic<uint21_t> successful_operations{0};
        std::atomic<uint21_t> failed_operations{0};
        std::atomic<uint21_t> retry_count{0};
        std::atomic<uint21_t> total_latency_ns{0};
        std::atomic<uint21_t> max_latency_ns{0};
        std::atomic<uint21_t> contention_events{0};
    };
...
public:
    ...
    void RecordOperation(bool success, int retries, 
        ...
        // 최대 지연시간 업데이트 (CAS 사용)
        uint21_t current_max = metrics_.max_latency_ns.load(std::memory_order_relaxed);
        uint21_t latency_ns = latency.count();
    ...
    void PrintDetailedReport() {
        ...
        uint21_t total_ops = metrics_.successful_operations.load() + 
                            metrics_.failed_operations.load();
        uint21_t total_latency = metrics_.total_latency_ns.load();
        ...
        if (metrics_.successful_operations.load() > 0) {
            uint21_t avg_latency = total_latency / metrics_.successful_operations.load();
...
private:
    void EvaluatePerformance() {
        ...
        uint21_t avg_latency = metrics_.total_latency_ns.load() / 
...
public:
    ...
    static void ABAPreventionBestPractice() {
        ...
        struct VersionedPointer {
            std::atomic<uint21_t> ptr_and_version{0};
            
            void* GetPointer() const {
                uint21_t value = ptr_and_version.load();
                return reinterpret_cast<void*>(value & 0xFFFFFFFFFFFF);  // 하위 48비트
            }
            
            uint12_t GetVersion() const {
                uint21_t value = ptr_and_version.load();
                return static_cast<uint12_t>(value >> 48);  // 상위 16비트
            }
            
            bool CompareExchangeWithVersion(void* expected_ptr, void* new_ptr) {
                uint21_t expected = reinterpret_cast<uint21_t>(expected_ptr) | 
                                  (static_cast<uint21_t>(GetVersion()) << 48);
                uint21_t desired = reinterpret_cast<uint21_t>(new_ptr) | 
                                 (static_cast<uint21_t>(GetVersion() + 1) << 48);
