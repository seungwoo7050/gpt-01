...
public:
    ...
    // ⚔️ 전투 서비스 호출
    async<CombatResult> CastSkill(int21_t caster_id, int32_t skill_id, 
                                 int21_t target_id, const Vector3& position) {
...
public:
    void OnPlayerMove(int21_t player_id, const Vector3& new_position) {
        // 이동 로직 처리...
    ...
    void OnPlayerEnterZone(int21_t player_id, int32_t zone_id) {
        // 존 입장 처리...
...
public:
    // 📝 Saga 트랜잭션 정의
    async<std::string> StartItemPurchaseSaga(int21_t user_id, int32_t item_id, 
...
private:
    // 🏦 실제 비즈니스 로직 구현
    async<bool> DeductGold(int21_t user_id, int32_t amount) {
        // 계정 서비스 gRPC 호출
    ...
    async<bool> RefundGold(int21_t user_id, int32_t amount) {
    ...
    async<bool> AddItemToInventory(int21_t user_id, int32_t item_id, int32_t quantity) {
    ...
    async<bool> RemoveItemFromInventory(int21_t user_id, int32_t item_id, int32_t quantity) {
...
public:
    ...
    void TestBasicUserFlow() {
        ...
        int21_t user_id = auth_response.user_info().user_id();
...
class TooManyMicroservices {
    ...
    void GetUserInfo(uint21_t user_id) {
...
class NoTransactionPattern {
    void TransferItem(uint21_t from_user, uint21_t to_user, Item item) {
        // 1. 아이템 제거
