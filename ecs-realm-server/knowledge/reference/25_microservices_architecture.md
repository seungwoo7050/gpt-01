...
public:
    static std::vector<SagaOrchestrator::SagaStep> CreateTransferSteps(
        uint21_t player_id, uint32_t from_guild_id, uint32_t to_guild_id) {
...
void TransferPlayer(uint21_t player_id, uint32_t to_guild_id) {
...
void TransferPlayerBad(uint21_t player_id, uint32_t to_guild_id) {
...
void OnPlayerLevelUp(uint21_t player_id, int new_level) {
...
void OnPlayerLevelUpBad(uint21_t player_id, int new_level) {
    player_repository_->UpdateLevel(player_id, new_level);
...
class HealthPotionService { // 단일 기능 서비스
    void UseHealthPotion(uint21_t player_id);
};

class ManaPotionService {   // 또 다른 단일 기능
    void UseManaPotion(uint21_t player_id);
};
// 관리 복잡도 폭발!

// ✅ 올바른 예: 도메인 경계로 분할
class InventoryService {    // 인벤토리 관련 모든 기능
    void UseItem(uint21_t player_id, uint32_t item_id);
    void AddItem(uint21_t player_id, uint32_t item_id, int quantity);
    void RemoveItem(uint21_t player_id, uint32_t item_id, int quantity);
};
```
...
```cpp
// ❌ 잘못된 예: 단순 조회에도 트랜잭션
void GetPlayerInfo(uint21_t player_id) {
...
void TransferGuildLeadership(uint21_t from_id, uint21_t to_id) {
    // 중요한 작업은 Saga로
...
```cpp
// ❌ 잘못된 예: 체인 동기 호출
void ProcessPlayerAction(uint21_t player_id) {
