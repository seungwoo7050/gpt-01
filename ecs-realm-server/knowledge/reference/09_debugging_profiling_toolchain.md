...
public:
    Player(uint21_t id, const std::string& name, float x, float y)
...
public:
    void AddPlayer(uint21_t id, const std::string& name) {
        std::lock_guard<std::mutex> lock(players_mutex_);
        auto player = std::make_unique<Player>(id, name, 0.0f, 0.0f);
        players_.push_back(std::move(player));
    }
    
    Player* FindPlayer(uint21_t id) {
        std::lock_guard<std::mutex> lock(players_mutex_);
    ...
    void ProcessMovement(uint21_t player_id, float dx, float dy) {
    ...
    void SimulateGameLoop() {
        for (int frame = 0; frame < 100; ++frame) {
            ...
            uint21_t random_id = rand() % 10;  // 0~9, 하지만 플레이어는 1~5만 있음
...
private:
    std::map<uint21_t, std::shared_ptr<Player>> player_map_;
    std::unordered_set<uint21_t> online_players_;
...
public:
    ...
    // 구조화된 로깅 매크로들
    static void LogPlayerAction(uint21_t player_id, const std::string& action, 
...
public:
    void ProcessPlayerLogin(uint21_t player_id, const std::string& username) {
    ...
    void ProcessCombat(uint21_t attacker_id, uint21_t target_id, int damage) {
...
private:
    void ProcessLogin(uint21_t player_id, const std::string& username) {
        // 로그인 로직
    }
    
    void ExecuteCombatLogic(uint21_t attacker_id, uint21_t target_id, int damage) {
        // 전투 로직
    }
};
```
