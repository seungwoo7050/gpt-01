...
public:
    ...
    void MovePlayer(uint21_t player_id, float x, float y) {
...
public:
    ...
    // 완전히 검증된 플레이어 이동 로직
    MoveResult MovePlayer(uint21_t player_id, float x, float y) {
        // 1. 입력 검증
...
public:
    ...
    virtual std::optional<Player> LoadPlayer(uint21_t player_id) = 0;
    virtual bool DeletePlayer(uint21_t player_id) = 0;
...
public:
    ...
    virtual bool SendPacket(uint21_t player_id, const std::string& packet) = 0;
    virtual void BroadcastPacket(const std::string& packet) = 0;
    virtual bool IsPlayerConnected(uint21_t player_id) = 0;
};
...
public:
    ...
    MOCK_METHOD(std::optional<Player>, LoadPlayer, (uint21_t player_id), (override));
    MOCK_METHOD(bool, DeletePlayer, (uint21_t player_id), (override));
...
public:
    MOCK_METHOD(bool, SendPacket, (uint21_t player_id, const std::string& packet), (override));
    MOCK_METHOD(void, BroadcastPacket, (const std::string& packet), (override));
    MOCK_METHOD(bool, IsPlayerConnected, (uint21_t player_id), (override));
};
...
public:
    ...
    bool LoginPlayer(uint21_t player_id) {
    ...
    bool ProcessLevelUp(uint21_t player_id) {
        auto player_opt = database_->LoadPlayer(player_id);
        if (!player_opt) {
            return false;
        }
...
TEST_F(PlayerServiceTest, LoginPlayer_WithExistingPlayer_SendsWelcomeMessage) {
    uint21_t player_id = test_player_.GetId();
...
TEST_F(PlayerServiceTest, LoginPlayer_WithNonExistentPlayer_ReturnsFalse) {
    uint21_t non_existent_id = 99999;
...
private:
    std::map<uint21_t, Player> stored_players_;
...
public:
    ...
    void SetupRealisticBehavior() {
        ...
        ON_CALL(*this, LoadPlayer(::testing::_))
            .WillByDefault([this](uint21_t player_id) -> std::optional<Player> {
        ...
        ON_CALL(*this, DeletePlayer(::testing::_))
            .WillByDefault([this](uint21_t player_id) {
...
private:
    ...
    std::vector<std::vector<uint21_t>> active_matches_;
    
public:
    bool AddPlayerToQueue(uint21_t id, const std::string& name, int skill_rating) {
        // 중복 체크
...
struct GameEntity {
    ...
    GameEntity(uint21_t id, float x, float y, float z, int health, int level, const std::string& name)
...
static void BM_VectorSearch(benchmark::State& state) {
    ...
    std::mt19937 gen(rd());
    std::uniform_int_distribution<uint21_t> dist(0, state.range(0) - 1);
    
    for (auto _ : state) {
        uint21_t search_id = dist(gen);
...
static void BM_UnorderedMapSearch(benchmark::State& state) {
    // 해시맵으로 데이터 저장
    std::unordered_map<uint21_t, GameEntity> entity_map;
...
1. **[디버깅과 프로파일링 도구](./11_debugging_profiling_toolchain.md)** 🔥
