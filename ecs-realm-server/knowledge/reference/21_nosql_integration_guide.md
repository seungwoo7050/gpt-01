...
public:
    ...
    bool SavePlayerInventory(uint21_t player_id, const nlohmann::json& inventory) {
        try {
            ...
            auto doc = builder 
                << "player_id" << static_cast<int21_t>(player_id)
...
public:
    ...
    bool UpdateQuestProgress(uint21_t player_id, uint32_t quest_id, 
...
public:
    ...
    void TrackPlayerAction(uint21_t player_id, const std::string& action, 
    ...
    PlayerBehaviorAnalytics GetPlayerBehavior(uint21_t player_id, 
...
public:
    ...
    bool TransferCharacterToGuild(uint21_t player_id, uint32_t guild_id) {
    ...
    void SynchronizePlayerData(uint21_t player_id) {
...
class PlayerDataStrategy {
    void SavePlayerLogin(uint21_t player_id) {
...
class ResilientDataAccess {
    std::optional<PlayerData> GetPlayerData(uint21_t player_id) {
        // Try cache first
...
void TransferItem(uint21_t from_player, uint21_t to_player, uint32_t item_id) {
    mysql_->RemoveItem(from_player, item_id);  // 성공
    mongo_->AddItem(to_player, item_id);       // 실패! 아이템 소멸
}

// ✅ 올바른 예: 보상 트랜잭션 패턴
void TransferItemSafely(uint21_t from_player, uint21_t to_player, uint32_t item_id) {
    // 2단계 커밋 패턴
