...
public:
    ...
    void HandleMovement(uint21_t player_id, float x, float y) {
        // 문제 2: 클라이언트 입력을 그대로 신뢰
        SetPlayerPosition(player_id, x, y);  // 😱 검증 없음!
        // 결과: 벽 뚫기, 순간이동 치트 가능
    }
    
    void HandleItemPurchase(uint21_t player_id, int item_id, int quantity, int price) {
    ...
    void HandleChatMessage(uint21_t player_id, const std::string& message) {
        // 문제 4: SQL 인젝션 취약점
...
public:
    ...
    void HandleMovement(const std::string& jwt_token, float x, float y) {
        ...
        uint21_t player_id = claims->player_id;
...
std::string JWTManager::Base64UrlEncode(const std::string& input) {
    ...
    BIO_set_flags(b64, BIO_FLAGS_BASE21_NO_NL);
...
std::string JWTManager::CreateAccessToken(uint21_t player_id, const std::string& username) {
    JWTClaims claims;
...
std::string JWTManager::CreateRefreshToken(uint21_t player_id, const std::string& username) {
    JWTClaims claims;
    claims.player_id = player_id;
    claims.username = username;
    claims.issued_at = std::chrono::system_clock::now();
...
struct SessionData {
    ...
    uint21_t player_id;                                  // 플레이어 고유 ID
    uint21_t character_id;                               // 선택된 캐릭터 ID (0=미선택)
...
std::string SessionManager::CreateSession(uint21_t player_id, const std::string& ip_address) {
    // 입력 검증
    if (player_id == 0) {
...
std::vector<SessionData> SessionManager::GetPlayerSessions(uint21_t player_id) {
    auto conn = redis_pool_->GetConnection();
    if (!conn) return {};
...
private:
    ...
    struct SecurityStats {
        std::atomic<uint21_t> failed_logins_per_ip[256];  // IP별 실패 횟수
        std::atomic<uint21_t> suspicious_patterns;        // 의심스러운 패턴 수
...
private:
    ...
    void TriggerSecurityAlert(const std::string& alert_type, const std::string& source, uint21_t count) {
...
struct ProductionSecurityMetrics {
    ...
    struct JWTPerformance {
        ...
        uint21_t tokens_validated_per_second = 5000; // 초당 검증 수
        double validation_success_rate = 99.97;      // 검증 성공률
    };
    
    // 세션 관리 성능  
    struct SessionPerformance {
        uint21_t active_sessions = 4850;            // 활성 세션 수
        double session_lookup_time_ms = 0.15;      // 세션 조회 시간
        double session_cleanup_efficiency = 98.5;   // 정리 효율성 (%)
        uint21_t sessions_per_server = 1616;       // 서버당 세션 수
    };
    
    // Rate Limiting 효과
    struct RateLimitingStats {
        uint21_t requests_blocked_per_hour = 2847;  // 시간당 차단 요청
        double false_positive_rate = 0.02;         // 오탐율 (%)
        uint21_t ddos_attempts_blocked = 23;       // 차단된 DDoS 시도
        double legitimate_requests_affected = 0.001; // 영향받은 정상 요청 (%)
    };
    
    // 보안 이벤트 현황
    struct SecurityEvents {
        uint21_t failed_login_attempts_daily = 1247;  // 일일 로그인 실패
        uint21_t suspicious_ips_blocked = 89;         // 차단된 의심 IP
        uint21_t token_tampering_attempts = 12;       // 토큰 조작 시도
        uint21_t session_hijacking_attempts = 3;      // 세션 하이재킹 시도
    };
};
```
...
public:
    ...
    bool DetectCheating(uint21_t player_id, const std::string& action, 
