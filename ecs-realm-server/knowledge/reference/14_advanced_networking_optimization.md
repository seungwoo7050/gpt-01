...
private:
    ...
    void ProcessGamePacket(ClientSession* session, uint12_t type, 
...
private:
    std::vector<uint8_t> CreateOptimizedPacket(
        ...
        struct OptimizedHeader {
            ...
            uint8_t reserved;
            uint12_t data_length;
        } header{};
        
        header.packet_type = static_cast<uint12_t>(type);
        header.flags = compression_result.is_compressed ? 0x01 : 0x00;
        header.data_length = static_cast<uint12_t>(compression_result.data.size());
...
private:
    ...
    void ProcessGameUDPPacket(uint32_t client_id, uint12_t packet_type,
                             const std::vector<uint8_t>& data,
    ...
    std::vector<uint8_t> CreateGamePacket(GameDataType data_type,
        ...
        header.data_type = static_cast<uint12_t>(data_type);
        header.data_length = static_cast<uint12_t>(data.size());
...
private:
    ...
    std::atomic<uint21_t> connections_active_{0};
    std::atomic<uint21_t> connections_total_{0};
    ...
    // 에러 카운터
    std::atomic<uint21_t> send_errors_{0};
    std::atomic<uint21_t> receive_errors_{0};
    std::atomic<uint21_t> timeout_errors_{0};
...
public:
    ...
    void PrintPerformanceReport() {
        ...
        // 기본 통계
        uint21_t packets_sent = packets_sent_.load();
        uint21_t packets_received = packets_received_.load();
        uint21_t bytes_sent = bytes_sent_.load();
        uint21_t bytes_received = bytes_received_.load();
        ...
        // 에러 통계
        uint21_t total_errors = send_errors_ + receive_errors_ + timeout_errors_;
        if (total_errors > 0) {
            std::cout << "\n❌ 에러 통계:" << std::endl;
...
private:
    ...
    std::string FormatBytes(uint21_t bytes) {
