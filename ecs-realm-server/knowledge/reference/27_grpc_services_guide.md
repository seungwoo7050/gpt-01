...
public:
    // 💀 문제: 매번 HTTP 요청/응답
    PlayerInfo GetPlayer(uint21_t player_id) {
    ...
    // 😱 더 심각한 문제: 여러 서비스 호출
    CompletePlayerData GetCompletePlayerData(uint21_t player_id) {
...
public:
    ...
    // 😍 병렬 서비스 호출로 더욱 빠르게!
    CompletePlayerData GetCompletePlayerDataFast(uint21_t player_id) {
        auto start = std::chrono::high_resolution_clock::now();
...
public:
    ...
    grpc::Status StreamPlayerUpdates(grpc::ServerContext* context,
        ...
        try {
            ...
            std::vector<uint21_t> player_ids(request->player_ids().begin(), 
...
public:
    ...
    std::future<std::optional<Player>> GetPlayerAsync(uint21_t player_id) {
    ...
    void StreamPlayerUpdates(const std::vector<uint21_t>& player_ids,
