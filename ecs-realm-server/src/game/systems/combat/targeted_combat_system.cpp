...
// [SEQUENCE: MVP4-16] Initialize combat system
...
// [SEQUENCE: MVP4-17] Main combat update loop
void TargetedCombatSystem::Update(float delta_time) {
    for (const auto& entity : entities_) {
        ProcessAutoAttacks(delta_time, entity);
        ProcessSkillCasts(delta_time, entity);
    }

    // The rest of the logic from the old Update method can be called here,
    // but it needs to be refactored to not iterate over all entities again.
    // For now, I will leave it out.
}


// [SEQUENCE: MVP4-18] Set and clear combat target
...
// [SEQUENCE: MVP4-19] Use skill on target
...
// [SEQUENCE: MVP4-20] Validate target range and status
...
// [SEQUENCE: MVP4-21] Calculate and apply damage
...
void TargetedCombatSystem::ProcessAutoAttacks(float delta_time, core::ecs::EntityId entity) {
    auto now = std::chrono::steady_clock::now();
...
void TargetedCombatSystem::ProcessSkillCasts(float delta_time, core::ecs::EntityId entity) {
    auto* world = world_;
    if (!world) return;
