...
// [SEQUENCE: 1] Initialize action combat system
...
void ActionCombatSystem::Update(float delta_time) {
    // Update projectiles at high frequency
...
// [SEQUENCE: 4] Use skillshot ability
...
// [SEQUENCE: 5] Use ground-targeted AoE
...
// [SEQUENCE: 7] Create projectile entity
core::ecs::EntityId ActionCombatSystem::CreateProjectile(core::ecs::EntityId owner,
    ...
    projectiles_[projectile] = proj_comp;
...
// [SEQUENCE: 8] Get entities in cone
...
// [SEQUENCE: 9] Dodge roll
bool ActionCombatSystem::DodgeRoll(core::ecs::EntityId entity, const core::utils::Vector3& direction) {
    auto* world = GetWorld();
    if (!world) return false;
    
    auto& dodge_state = dodge_states_[entity];
    
    // Check if can dodge
    auto now = std::chrono::steady_clock::now();
    if (dodge_state.is_dodging || dodge_state.dodge_charges <= 0) {
        return false;
    }
    
    if (now < dodge_state.next_dodge_time) {
        return false;
    }
    
    // Start dodge
    dodge_state.is_dodging = true;
    dodge_state.dodge_end_time = now + std::chrono::milliseconds(
        static_cast<int>(config_.dodge_duration * 1000)
    );
    dodge_state.dodge_direction = direction;
    dodge_state.dodge_charges--;
    dodge_state.next_dodge_time = now + std::chrono::milliseconds(500);  // 0.5s between dodges
...
// [SEQUENCE: 10] Check if invulnerable
...
// [SEQUENCE: 11] Calculate damage (same as targeted)
...
// [SEQUENCE: 13] Update projectiles
void ActionCombatSystem::UpdateProjectiles(float delta_time) {
    ...
    for (auto& [projectile_id, proj] : projectiles_) {
        auto* transform = world->GetComponent<TransformComponent>(projectile_id);
        if (!transform) {
            to_remove.push_back(projectile_id);
            continue;
        }
        ...
        // Check collisions
        CheckProjectileCollisions(projectile_id, proj, transform->position);
    }
...
// [SEQUENCE: 14] Check projectile collisions
...
// [SEQUENCE: 15] Process skill casts
void ActionCombatSystem::ProcessSkillCasts(float delta_time) {
    // Similar to targeted system but for action skills
    auto* world = GetWorld();
    if (!world) return;
...
// [SEQUENCE: 16] Process cooldowns
void ActionCombatSystem::ProcessSkillCooldowns(float delta_time) {
    auto* world = GetWorld();
    if (!world) return;
...
// [SEQUENCE: 17] Update dodge states
