...
### [SEQUENCE: MVP1-53] 기술 면접 대비 심층 분석 (In-depth Analysis for Technical Interviews)

#### 1. 비동기 네트워크 엔진: Boost.Asio
*   **기술 선택 이유 (Why Boost.Asio?):** 고성능 게임 서버는 수천, 수만 명의 동시 접속을 처리해야 하므로, 하나의 스레드가 하나의 연결을 막는(blocking) 동기 방식은 부적합합니다. Boost.Asio는 Proactor 패턴 기반의 비동기 I/O 모델을 사용하여, 소수의 스레드(보통 CPU 코어 수와 동일)만으로 수많은 클라이언트의 요청을 효율적으로 처리할 수 있습니다. 또한 C++ 표준 라이브러리(`std::net`)의 기반이 되는 기술이라는 점에서 기술적 정당성과 미래 확장성을 모두 확보할 수 있었습니다.
*   **다른 대안과 비교 (Trade-offs):**
    *   **Raw Sockets (e.g., `epoll`, `kqueue`):** OS가 제공하는 가장 저수준의 API로 최고의 성능을 낼 수 있지만, 직접 구현하기에는 매우 복잡하고 플랫폼 종속적인 코드를 작성해야 합니다. Boost.Asio는 이러한 저수준 API를 잘 추상화하여 생산성과 성능의 균형을 맞춘 훌륭한 대안입니다.
    *   **libevent, libuv:** C 기반의 성숙한 비동기 이벤트 라이브러리들입니다. 안정적이지만, 현대적인 C++의 관용구(RAII, 스마트 포인터 등)와는 다소 이질적인 C 스타일의 API를 사용해야 합니다.
*   **구현 시 어려웠던 점:**
    *   **객체 생명주기 관리:** 비동기 작업(e.g., `async_read`)을 요청하고 콜백 함수가 호출되기 전에 `Session` 객체가 파괴되는 것을 막는 것이 가장 어렵고 중요한 문제입니다. `std::shared_ptr`와 `std::enable_shared_from_this`를 사용하여, 비동기 작업이 진행 중인 동안에는 객체의 생명주기가 안전하게 유지되도록 보장해야 했습니다. 이 부분에서 발생하는 버그는 찾기가 매우 까다롭습니다.
    *   **스레드 안정성:** `SessionManager`와 같이 여러 스레드에서 동시에 접근할 수 있는 공유 자원은 `std::shared_mutex` 등을 사용하여 반드시 동기화해야 합니다. 읽기 작업은 여러 스레드가 동시에 접근하도록 허용하고, 쓰기 작업은 단 하나의 스레드만 접근하도록 제어하여 성능과 안정성을 모두 고려했습니다.

#### 2. 데이터 직렬화: Protocol Buffers
*   **기술 선택 이유 (Why Protobuf?):** 클라이언트와 서버가 명확한 규격(.proto 파일)에 따라 통신하도록 강제하여 호환성 문제를 줄일 수 있습니다. 또한, 텍스트 기반인 JSON에 비해 바이너리 직렬화를 통해 훨씬 작은 패킷 크기와 빠른 직렬화/역직렬화 속도를 제공하여 게임 서버의 네트워크 성능에 기여합니다.
*   **다른 대안과 비교 (Trade-offs):**
    *   **JSON:** 사람이 읽기 쉬워 디버깅에 유리하지만, 파싱이 느리고 데이터 크기가 커서 실시간 게임 서버에는 부적합한 경우가 많습니다.
    *   **FlatBuffers:** Protobuf와 유사하지만, 데이터를 파싱(역직렬화)하는 과정 없이 메모리에 올라온 데이터를 바로 읽을 수 있는 'Zero-copy' 특성을 가집니다. 이는 극단적인 성능 최적화가 필요할 때 최고의 선택지가 될 수 있지만, 사용법이 다소 복잡하고 유연성이 떨어지는 단점이 있습니다. 이 프로젝트에서는 Protobuf의 성능으로도 충분하다고 판단했습니다.
*   **만약 다시 만든다면:** 패킷 헤더에 패킷 ID뿐만 아니라, 요청/응답을 구분하기 위한 시퀀스 ID를 추가하는 것을 고려할 것입니다. 이를 통해 클라이언트는 자신이 보낸 요청에 대한 응답을 더 명확하게 매칭할 수 있어, 비동기 통신 로직을 더 견고하게 만들 수 있습니다.